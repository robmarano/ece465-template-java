...
SET NODE=1
SET NAME=zkApp%NODE%
docker run -d --name %NAME% --rm=true --hostname %NAME% --publish 218%NODE%:2181 --publish 906%NODE%:8080 --publish 907%NODE%:8081 --env-file zookeeper%NODE%.env -v zookeeper%NODE%-dataDir:/data -v zookeeper%NODE%-dataLogDir:/datalog -v zookeeper%NODE%-logs:/logs --network zookeeper-cluster %IMAGE%:%VERSION%
